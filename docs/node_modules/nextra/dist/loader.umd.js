(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('path'), require('fs'), require('loader-utils'), require('gray-matter'), require('slash')) :
  typeof define === 'function' && define.amd ? define(['path', 'fs', 'loader-utils', 'gray-matter', 'slash'], factory) :
  (global = global || self, global.nextra = factory(global.path, global.fs, global.loaderUtils, global.grayMatter, global.slash));
}(this, (function (path, fs, loaderUtils, grayMatter, slash) {
  ...
  async function analyzeLocalizedEntries(currentResourcePath, defaultLocale) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const content = await fs.promises.readFile(path.join(dir, file.name), 'utf-8');
  ...
  async function loader (source) {
    ...
    const {
      resourcePath,
      resourceQuery
    } = this; // Add the entire directory `pages` as the dependency
    // so we can generate the correct page map

    this.addContextDependency(path.resolve('pages')); // Generate the page map

    let [pageMap, route] = await getPageMap(resourcePath); // Extract frontMatter information if it exists
    ...
    source = content; // Check if there's a theme provided
