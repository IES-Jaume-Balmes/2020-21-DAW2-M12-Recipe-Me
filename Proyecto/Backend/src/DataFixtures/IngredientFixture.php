<?php

namespace App\DataFixtures;

use App\Entity\Ingredient;
use Doctrine\Persistence\ObjectManager;
use Doctrine\Bundle\FixturesBundle\Fixture;

class IngredientFixture extends Fixture
{
    public const INGREDIENT_REFERENCE = 'ingredient-';

    public function load(ObjectManager $manager)
    {
        // CARGAMOS INGREDIENTES
        /*for ($i = 0; $i < 40; $i++) {
            $faker = \Faker\Factory::create();
            $faker->addProvider(new \Bezhanov\Faker\Provider\Food($faker));

            $ingrediente = new Ingredient();
            $ingrediente->setName($faker->ingredient);
            $manager->persist($ingrediente);
            $this->addReference(self::INGREDIENT_REFERENCE . $i, $ingrediente);
        }*/

        $ingredients = array('Aceite de algodón',
            'Aceite de cacahuete',
            'Aceite de coco',
            'Aceite de colza',
            'Aceite de germen de trigo',
            'Aceite de girasol',
            'Aceite de grano de uva',
            'Aceite de hígado de bacalao',
            'Aceite de lino',
            'Aceite de nuez',
            'Aceite de oliva',
            'Aceite de oliva virgen extra',
            'Aceite de oliva virgen extra, producción ecologica',
            'Aceite de palma',
            'Aceite de sésamo',
            'Aceite de soja',
            'Aceituna',
            'Aceituna negra, con hueso',
            'Acelga, cruda',
            'Acelga, en conserva',
            'Acelgas, hervidas',
            'Achicoria, cruda',
            'Agua de la red',
            'Agua mineral, mineralización debil',
            'Agua, con gas, embotellada',
            'Aguacate',
            'Aguacate congelado',
            'Aguardiente',
            'Ajo',
            'Ajo, en polvo',
            'Ajo, frito',
            'Albahaca',
            'Albaricoque',
            'Albondigas en conserva',
            'Alcachofa, cruda',
            'Alcachofas en conserva',
            'Alcaparra',
            'Alioli',
            'Almeja',
            'Almejas en conserva',
            'Almendra, cruda',
            'Almendra, cruda, con cáscara',
            'Almendra, frita, salada',
            'Almendra, tostada',
            'Almidón de arroz',
            'Almidón de maíz',
            'Almidón de trigo',
            'Altramuz',
            'Alubia blanca, en conserva',
            'Alubia blanca, seca, cruda',
            'Alubia negra, seca, remojada, hervida',
            'Anacardo, crudo',
            'Anchoas en aceite vegetal',
            'Anguila, al horno',
            'Anguila, cruda',
            'Anguila, hervida',
            'Angula, cruda',
            'Anís, seco',
            'Aperitivos de trigo',
            'Apio, crudo',
            'Apio, en conserva en salmuera',
            'Arandano',
            'Arenque, ahumado',
            'Arenque, salado',
            'Arroz',
            'Arroz con leche',
            'Arroz hinchado, para el desayuno, enriquecido',
            'Arroz integral, crudo',
            'Arroz integral, hervido',
            'Arroz, hervido',
            'Atún en aceite de oliva isabel',
            'Atun en aceite vegetal',
            'Atún en escabeche',
            'Atún, al horno',
            'Atún, al natural',
            'Atún, crudo',
            'Atún, plancha',
            'Avellana',
            'Avellana, cruda, con cáscara',
            'Avena, cruda',
            'Avestruz, solomillo, crudo',
            'Azafrán',
            'Azúcar blanca',
            'Azúcar, moreno',
            'Bacaladilla',
            'Bacalao, ahumado',
            'Bacalao, crudo',
            'Bacalao, fresco, al horno',
            'Bacalao, frito',
            'Bacalao, salado, crudo',
            'Bacalao, salado, remojado, crudo',
            'Bacón, ahumado, a la parrilla',
            'Bacon, crudo, con grasa separable',
            'Bambú, crudo',
            'Barra chocolate, tipo "bounty"',
            'Barra de chocolate, tipo "Kit-kat"',
            'Barra de chocolate, tipo "Mars"',
            'Barrita cereales con chocolate',
            'Barrita cereales con frutas',
            'Barrita cereales maíz y trigo',
            'Barrita cereales trigo y chocolate',
            'Batido de chocolate',
            'Batido de fresa',
            'Batido de soja',
            'Batido fermentado de soja',
            'Batido lácteo de cacao, bajo en calorías',
            'Batido lácteo, de fresa, bajo en calorías',
            'Bebida de soja',
            'Bebida energética',
            'Bebida isotónica',
            'Berberechos',
            'Berberechos en conserva',
            'Berenjena',
            'Berenjena, frita, en aceite de girasol',
            'Berro, crudo',
            'Besugo',
            'Bígaro, hervido',
            'Bizcocho',
            'Bocadillo',
            'Bogavante',
            'Bogavante, hervido',
            'Bollería, genérico',
            'Bombón',
            'Boniato, crudo',
            'Bonito del norte, al vapor',
            'Bonito del norte, enlatado en aceite de soja',
            'Bonito en aceite vegetal',
            'Bonito, enlatado en aceite, escurrido',
            'Boquerón',
            'Boquerón, frito',
            'Borraja',
            'Breca, cruda',
            'Brécol, crudo',
            'Brécol, hervido',
            'Butifarra',
            'Caballa, al horno',
            'Caballa, cruda',
            'Caballa, enlatada en aceite, escurrida',
            'Cabracho o rascacio, crudo',
            'Cabrito lechal, costillar, asado',
            'Cabrito lechal, costillar, crudo',
            'Cabrito lechal, espalda, cruda',
            'Cabrito lechal, pierna, cruda',
            'Cabrito, parte s/e, crudo, con grasa separable',
            'Cacahuete, crudo, con cáscara',
            'Cacahuete, frito, salado',
            'Cacahuete, tostado, salado',
            'Cacao en polvo, azucarado',
            'Cacao, en polvo, azucarado, bajo en calorías',
            'Café, con leche',
            'Café, en grano o molido, descafeinado',
            'Café, en polvo, sin reconstituir',
            'Café, en polvo, soluble',
            'Café, infusión',
            'Café, infusión, descafeinado',
            'Café, sucedáneo, soluble',
            'Calabacín',
            'Calabacín, asado',
            'Calabaza, cruda',
            'Calabaza, hervida',
            'Calamar, asado',
            'Calamar, conserva',
            'Calamar, crudo',
            'Calamares en aceite vegetal',
            'Caldo vegetal',
            'Callos de ternera',
            'Camarón',
            'Canela, en polvo',
            'Canelones',
            'Cangrejo',
            'Cangrejo de río crudo',
            'Cangrejo, en conserva',
            'Canonigos',
            'Caqui',
            'Caracol',
            'Caramelo',
            'Cardo',
            'Cardo, tallo, en conserva',
            'Carne de cerdo de capa blanca, cabezada, parte magra, cruda',
            'Carne de cerdo de capa blanca, cabezada, partes grasa y magra, crudo',
            'Carne de cerdo de capa blanca, chuleta, parte magra, cruda',
            'Carne de cerdo de capa blanca, chuleta, partes grasa y magra, cruda',
            'Carne de cerdo de capa blanca, jamón, parte magra, crudo',
            'Carne de cerdo de capa blanca, jamón, partes grasa y magra, crudo',
            'Carne de cerdo de capa blanca, lomo, parte magra, crudo',
            'Carne de cerdo de capa blanca, lomo, partes grasa y magra, crudo',
            'Carne de vacuno, cadera, partes magra y grasa, crudo',
            'Carne de vacuno, carrillera, partes magra y grasa, crudo',
            'Carne de vacuno, costillar, partes magra y grasa, crudo',
            'Carne de vacuno, lomo, partes magra y grasa, crudo',
            'Carne de vacuno, rabo, partes magra y grasa, crudo',
            'Carne de vacuno, redondo, partes magra y grasa, crudo',
            'Carne de vacuno, solomillo, partes magra y grasa, crudo',
            'Carne picada',
            'Carpa, al horno',
            'Carpa, cruda',
            'Castaña, cruda',
            'Castaña, tostada',
            'Cava',
            'Caviar',
            'Cebada, cruda',
            'Cebolla',
            'Cebolla, asada',
            'Cebolla, hervida',
            'Cebollino, crudo',
            'Cecina',
            'Centeno, crudo',
            'Centollo, crudo',
            'Cerdo, chuleta, crudo',
            'Cerdo, costilla, crudo',
            'Cerdo, lomo, asado',
            'Cerdo, lomo, crudo',
            'Cerdo, panceta, cruda',
            'Cerdo, parte sin especificar',
            'Cerdo, pierna, cruda, con grasa separable',
            'Cerdo, solomillo, asado',
            'Cerdo, solomillo, crudo',
            'Cereales base trigo y chocolate',
            'Cereales desayuno base de arroz chocolateado',
            'Cereales desayuno base de arroz y miel',
            'Cereales desayuno base de arroz, trigo y fruta',
            'Cereales desayuno base de maíz y miel',
            'Cereales desayuno base de maíz y trigo',
            'Cereales desayuno base de maíz, trigo y avena',
            'Cereales desayuno base de trigo azucarado',
            'Cereales desayuno base de trigo y arroz',
            'Cereales desayuno base de trigo y frutas',
            'Cereales desayuno base de trigo y miel',
            'Cereales desayuno base de trigo, avena, maiz y miel',
            'Cereales desayuno base de trigo, avena, maíz, miel y nueces',
            'Cereales desayuno base muesli',
            'Cereales desayuno, maíz azucarado',
            'Cereales para el desayuno, ricos en fibra, tipo "all-bran"',
            'Cereales, en polvo, solubles (eko)',
            'Cereza',
            'Cerveza con alcohol',
            'Cerveza sin alcohol',
            'Cerveza, baja en alcohol',
            'Cerveza, oscura, 8°- 9°',
            'Champiñon',
            'Champiñones en conserva',
            'Chayote, crudo',
            'Chicle, sin azúcar',
            'Chile, rojo',
            'Chile, verde',
            'Chipiron',
            'Chirimoya',
            'Chirla',
            'Chistorra',
            'Chocolate blanco',
            'Chocolate con leche',
            'Chocolate con leche y almendras',
            'Chocolate con leche y arroz',
            'Chocolate con nueces de macadamia',
            'Chocolate negro, con almendras',
            'Chocolate negro, con azúcar',
            'Chocolate, negro',
            'Choped pork',
            'Chorizo',
            'Chufa, cruda',
            'Churro',
            'Ciervo, parte s/e, crudo, con grasa separable',
            'Cigala',
            'Ciruela, con piel, cruda',
            'Ciruela, sin piel',
            'Coco, desecado',
            'Coco, fresco',
            'Col blanca, cruda',
            'Col de bruselas, cruda',
            'Col lombarda, hervida',
            'Col rizada, cruda',
            'Coliflor, congelada, cruda',
            'Coliflor, hervida',
            'Comino',
            'Complejo vitaminico',
            'Coñac',
            'Conejo de granja, carne, cruda',
            'Conejo, entero, estofado',
            'Confitura, frutas s/e, baja en calorías',
            'Congrio',
            'Corazón de cordero, crudo',
            'Corazón de pollo, crudo',
            'Corazon de vaca, crudo',
            'Corazón, de cerdo, crudo',
            'Corazón, de vaca/buey, cocido',
            'Corazón, de vaca/buey, crudo',
            'Cordero lechal, costillar, asado',
            'Cordero lechal, costillar, crudo',
            'Cordero lechal, espalda, cruda',
            'Cordero lechal, pierna, cruda',
            'Cordero ligero, costillar, asado',
            'Cordero ligero, costillar, crudo',
            'Cordero ligero, espalda, cruda',
            'Cordero ligero, pierna, cruda',
            'Cordero, costilla, crudo',
            'Cordero, parte sin especificar',
            'Cordero, pierna, con grasa, asada',
            'Corteza de trigo',
            'Crema catalana',
            'Crema de almendras',
            'Crema de cacahuete',
            'Crema de chocolate con avellanas',
            'Crema de chocolate y nata',
            'Crema de chocolate, baja en calorías',
            'Crema de coco',
            'Crema de vainilla y mousse de chocolate',
            'Crema inglesa',
            'Crema pastelera',
            'Cremoso san millan',
            'Croissant',
            'Croissant de chocolate',
            'Croquetas de pollo',
            'Cuajada',
            'Cubata',
            'Cubito de caldo',
            'Curry',
            'Dátil',
            'Donut',
            'Donut, de chocolate',
            'Dorada, cruda',
            'Dorada, plancha',
            'Empanada de carne',
            'Empanadilla',
            'Endibia',
            'Eneldo, seco',
            'Ensaimada',
            'Ensalada',
            'Ensaladilla rusa',
            'Escarola',
            'Espárrago, verde',
            'Esparragos blancos en conserva',
            'Espinaca, en conserva',
            'Espinaca, hervida',
            'Espinaca, picada, congelada, cruda',
            'Fabada',
            'Faneca, cruda',
            'Flamenquín',
            'Flan de huevo',
            'Flan de huevo, bajo en calorías',
            'Fletan, crudo',
            'Foie gras',
            'Frambuesa, cruda',
            'Fresa',
            'Fritos de maíz',
            'Fructosa',
            'Frutos secos',
            'Fuet',
            'Galleta ,tipo "digestiva", con chocolate',
            'Galleta salada',
            'Galleta, barquillo, con jalea de frutas',
            'Galleta, cubierta de chocolate',
            'Galleta, genérico',
            'Galleta, tipo "Digestiva"',
            'Galletas integrales',
            'Galletas saladas, con queso',
            'Galletas tipo maria',
            'Galletas, con chocolate, tipo "cookies"',
            'Galletas, de mantequilla',
            'Gallina, entera, cruda',
            'Gallo',
            'Gamba quisquilla, congelada',
            'Gamba roja, cruda',
            'Gamba, hervida',
            'Garbanzo, en conserva',
            'Garbanzo, hervido',
            'Garbanzo, seco, crudo',
            'Gazpacho',
            'Gelatina',
            'Germen de trigo',
            'Ginebra',
            'Gofio',
            'Gomasio',
            'Gominola, genérica',
            'Granada',
            'Grasa de pollo',
            'Grelo',
            'Grosella negra, cruda',
            'Grosella, cruda',
            'Guayaba, enlatada en almíbar',
            'Guayaba, sin piel, cruda',
            'Guindilla, en polvo',
            'Guindilla, picante',
            'Guisante, congelado, crudo',
            'Guisante, congelado, hervido',
            'Guisante, seco, crudo',
            'Guisantes en conserva',
            'Gusanito',
            'Haba, fresca',
            'Haba, frita',
            'Haba, seca',
            'Haba, seca, remojada, hervida',
            'Harina de avena',
            'Harina de avena, cocida en agua',
            'Harina de cebada',
            'Harina de centeno',
            'Harina de maíz',
            'Harina de soja',
            'Harina de trigo',
            'Harina de trigo, integral',
            'Helado de chocolate',
            'Helado de fresa',
            'Helado de nata',
            'Higado de pollo, crudo',
            'Hígado, de cerdo, crudo',
            'Hígado, de vaca/buey, crudo',
            'Higos y brevas',
            'Hinojo',
            'Hojaldre',
            'Horchata',
            'Huevo de codorniz, entero, crudo',
            'Huevo de gallina fresco',
            'Huevo de gallina, clara, cruda',
            'Huevo de gallina, escalfado',
            'Huevo de gallina, frito',
            'Huevo de gallina, hervido, duro',
            'Huevo de gallina, pasado por agua',
            'Huevo de gallina, revuelto, con mantequilla',
            'Huevo de gallina, yema, cruda',
            'Huevo de gallina, yema, desecada',
            'Huevo de pato, crudo',
            'Huevo de pavo, entero, crudo',
            'Infusión',
            'Jalea real',
            'Jamón asado',
            'Jamón cocido, categoría s/e',
            'Jamón cocido, enlatado',
            'Jamón ibérico de bellota',
            'Jamón ibérico de cebo',
            'Jamón serrano',
            'Jengibre',
            'Judía blanca',
            'Judía pinta',
            'Judía verde, congelada, cruda',
            'Judía verde, cruda',
            'Judia verde, hervida',
            'Judías blancas, cocidas',
            'Judias verdes en conserva',
            'Jurel, crudo',
            'Kebab',
            'Kefir',
            'Ketchup',
            'Kiwi',
            'Lacón',
            'Lactosa',
            'Langosta',
            'Langostino',
            'Lasaña',
            'Laurel, hoja',
            'Leche condensada, entera, con azúcar',
            'Leche de almendra',
            'Leche de cabra',
            'Leche de coco',
            'Leche de oveja',
            'Leche de vaca, desnatada, condensada, con azúcar',
            'Leche de vaca, entera',
            'Leche en polvo, semidesnatada',
            'Leche fermentada, bifidobacterium, entera, natural',
            'Leche materna',
            'Leche merengada',
            'Leche, desnatada, pasteurizada',
            'Leche, semidesnatada, pasteurizada',
            'Lechuga',
            'Lecitina de soja',
            'Lengua de cerdo, cruda',
            'Lengua de cordero, cruda',
            'Lengua, de buey, cruda',
            'Lengua, de ternera, cruda',
            'Lenguado',
            'Lenguado, al horno',
            'Lenguado, frito',
            'Lenguado, plancha',
            'Lenteja, en conserva',
            'Lenteja, hervida',
            'Lenteja, seca, cruda',
            'Licor apricot',
            'Licor benedictine',
            'Licor curaçao',
            'Licor de café',
            'Licor de crema 15-17% vol.',
            'Licores de frutas',
            'Liebre, entera, cruda',
            'Lija, cruda',
            'Lima, cruda',
            'Limonada',
            'Litchi, crudo',
            'Lombarda',
            'Lomo embuchado',
            'Longaniza',
            'Lubina',
            'Lucio, al horno',
            'Macedonia de frutas, conserva en su jugo',
            'Magdalena',
            'Maíz, en mazorca, congelado, crudo',
            'Maíz, en mazorca, crudo',
            'Mandarina',
            'Mandarina, congelada',
            'Mango, crudo',
            'Manteca',
            'Mantequilla salada',
            'Mantequilla, baja en calorías',
            'Manzana',
            'Margarina de maíz',
            'Margarina, baja en calorías',
            'Masa de hojaldre, cruda',
            'Mayonesa con aceite de girasol',
            'Mayonesa, aceite de oliva',
            'Mayonesa, aceite de soja',
            'Mayonesa, baja en calorías',
            'Mejillon',
            'Mejillón, en conserva, al natural',
            'Mejillón, en escabeche',
            'Mejillón, hervido',
            'Melocotón',
            'Melocotón en almíbar',
            'Melocotón, desecado',
            'Melón',
            'Membrillo, crudo',
            'Menestra de verduras, congelada',
            'Menta, fresca',
            'Merluza fresca',
            'Merluza, congelada, cruda',
            'Merluza, rebozada, frita',
            'Mermelada de albaricoque',
            'Mermelada de albaricoque y melocotón, baja en calorías',
            'Mermelada de ciruela',
            'Mermelada de frambuesa',
            'Mermelada de fresa',
            'Mermelada de fresa, baja en calorías',
            'Mermelada de grosella roja',
            'Mermelada de mora',
            'Mermelada de naranja',
            'Mero, crudo',
            'Mero, plancha',
            'Miel',
            'Mijo',
            'Miso',
            'Mojo picon',
            'Molleja de cordero',
            'Morcilla',
            'Morcilla, frita',
            'Mortadela',
            'Mostaza',
            'Mousse de chocolate',
            'Mousse de queso fresco, con frutas',
            'Mousse de queso fresco, desnatado, azucarado',
            'Mousse de queso fresco, desnatado, con frutas',
            'Mousse de yogur, con frutas',
            'Mousse de yogur, natural',
            'Muesli',
            'Mújol, al horno',
            'Mújol, crudo',
            'Nabo, pelado, crudo',
            'Napolitana, rellena con crema de cacao',
            'Naranja',
            'Nata montada',
            'Nata, líquida, para cocinar, 18% de grasa',
            'Natillas sabor vainilla',
            'Nécora',
            'Néctar de albaricoque, envasado',
            'Néctar de ciruela',
            'Néctar de frutas exóticas, envasado',
            'Néctar de mango, envasado',
            'Néctar de maracuyá, envasado',
            'Néctar de melocotón',
            'Néctar de naranja, envasado',
            'Néctar de pera, envasado',
            'Néctar de piña',
            'Néctar de pomelo',
            'Nectarina',
            'Níscalo, crudo',
            'Níspero, con piel, crudo',
            'Nispero, conserva en su jugo',
            'Nispero, sin piel , congelado',
            'Nuez',
            'Nuez moscada',
            'Nuez, cruda, con cáscara',
            'Oca, sin piel, asada',
            'Orégano, seco',
            'Ostra, cruda',
            'Paella',
            'Paella marinera',
            'Palmera',
            'Palmito, en conserva',
            'Palometa, cruda',
            'Palomitas de maíz, sin aceite, sin sal',
            'Pan blanco, de barra',
            'Pan blanco, de barra, sin sal',
            'Pan blanco, de molde, tostado',
            'Pan blanco, frito',
            'Pan blanco, tipo "baguette"',
            'Pan blanco, tostado',
            'Pan blanco, tostado sin sal',
            'Pan de avena',
            'Pan de cebada',
            'Pan de centeno',
            'Pan de leche',
            'Pan de maíz',
            'Pan integral',
            'Pan integral, de molde, tostado',
            'Pan integral, sin sal',
            'Pan integral, tostado',
            'Pan rallado',
            'Pan tostado integral',
            'Pan, tipo hamburguesa',
            'Panceta, frita',
            'Panga, cruda',
            'Papaya, cruda',
            'Paraguaya',
            'Pasta alimenticia, con huevo, cruda',
            'Pasta alimenticia, con huevo, hervida',
            'Pasta alimenticia, con vegetales, cruda',
            'Pasta alimenticia, cruda',
            'Pasta alimenticia, integral, cruda',
            'Pasta alimenticia, integral, hervida',
            'Pasta alimenticia, rellena con carne, hervida',
            'Pasta de fruta',
            'Pasta de sésamo',
            'Pastas de te',
            'Pastel',
            'Pastel con fruta confitada',
            'Pastel de chocolate',
            'Pastel de manzana',
            'Patata, asada',
            'Patata, cruda',
            'Patata, frita con aceite s/e, sin sal',
            'Patata, hervida',
            'Patata, prefrita, congelada',
            'Patata, salteada con aceite girasol',
            'Patatas chips, "lights"',
            'Patatas, fritas, chips',
            'Pate de higado de cerdo, 30% de grasa',
            'Paté de pimienta',
            'Pato, entero, asado',
            'Pato, entero, crudo',
            'Pavo, crudo',
            'Pavo, fiambre',
            'Pavo, fiambre, bajo en grasa',
            'Pavo, muslo, con piel, crudo',
            'Pavo, pechuga, con piel, crudo',
            'Pavo, pechuga, sin piel, a la plancha',
            'Pepinillos en vinagre',
            'Pepino',
            'Pera',
            'Pera, enlatada en almíbar',
            'Perca, al horno',
            'Perca, cruda',
            'Percebe',
            'Perdiz, cruda',
            'Perejil, fresco',
            'Pescadilla, congelada, cruda',
            'Pescadilla, rebozada en harina, frita',
            'Pescado, genérico',
            'Petit líquido, sabor fresa',
            'Petit suïsse, cereales y fruta',
            'Petit suisse, chocolate',
            'Petit suisse, fresa',
            'Petit Suisse, natural azucarado',
            'Pez espada, crudo',
            'Pez espada, emperador, plancha',
            'Pichón, sin piel, asado',
            'Pijota, cruda',
            'Pimentón, en polvo',
            'Pimienta, blanca',
            'Pimienta, negra',
            'Pimiento morrón, en conserva',
            'Pimiento rojo, crudo',
            'Pimiento, frito',
            'Piña',
            'Piña en almíbar',
            'Piña, enlatada en su jugo',
            'Piñon',
            'Piñón, crudo, con cáscara',
            'Pipa de calabaza',
            'Pipa de girasol',
            'Pipas de girasol, peladas, con sal',
            'Pistacho',
            'Pizza, precocinada',
            'Plátano',
            'Platija, al vapor',
            'Platija, cruda',
            'Pollo empanado, frito',
            'Pollo, ala, con piel, cruda',
            'Pollo, entero, con piel, asado',
            'Pollo, entero, con piel, crudo',
            'Pollo, fiambre',
            'Pollo, fiambre, bajo en grasa',
            'Pollo, frito',
            'Pollo, muslo, con piel, asado',
            'Pollo, muslo, con piel, crudo',
            'Pollo, parte sin especificar',
            'Pollo, pechuga, con piel, crudo',
            'Pollo, pechuga, plancha',
            'Pomelo',
            'Preparado lácteo con omega 3',
            'Pudding de pasas',
            'Puerro, crudo y congelado',
            'Puerro, en conserva',
            'Pulmón, de cerdo, crudo',
            'Pulmón, de cordero, crudo',
            'Pulmón, de ternera, crudo',
            'Pulpo',
            'Pulpo, hervido',
            'Puré de patata y queso, en copos',
            'Puré de patata, con verduras, en copos',
            'Puré de patata, en copos',
            'Puré de patatas y champiñones, en copos',
            'Puré, de patata, con leche',
            'Queso ahumado de aliva',
            'Queso azul',
            'Queso brie',
            'Queso cabra, curado',
            'Queso cabrales',
            'Queso Camembert 20-30% MG/ES',
            'Queso camembert 60% mg/es',
            'Queso cantabria',
            'Queso casín',
            'Queso Castellano',
            'Queso cebreiro',
            'Queso Cheddar',
            'Queso curado, genérico',
            'Queso de aracena',
            'Queso de arzúa',
            'Queso de bola',
            'Queso de cádiz',
            'Queso de Castilla-La Mancha, oveja y cabra',
            'Queso de Castilla-La Mancha, oveja, vaca y cabra',
            'Queso de castilla-león, oveja y vaca',
            'Queso de murcia, al vino',
            'Queso de tenerife',
            'Queso de tiétar',
            'Queso edam',
            'Queso emmental',
            'Queso en porciones',
            'Queso flor de guía',
            'Queso fresco de burgos',
            'Queso fresco, cabra',
            'Queso fresco, desnatado, con frutas',
            'Queso fundido, extragraso (>60% MG/ES)',
            'Queso gata-hurdes',
            'Queso gaztazarra',
            'Queso Gouda',
            'Queso grazalema',
            'Queso Gruyer',
            'Queso herreño',
            'Queso Ibores',
            'Queso Mahón',
            'Queso majorero',
            'Queso manchego',
            'Queso manchego, en aceite',
            'Queso mozzarella',
            'Queso munster',
            'Queso para untar',
            'Queso para untar, con finas hierbas',
            'Queso para untar, con salmón',
            'Queso para untar, natural, bajo en calorías',
            'Queso parmesano',
            'Queso pasiego',
            'Queso pedroches',
            'Queso peñamellera',
            'Queso picón',
            'Queso porrúa',
            'Queso Raclette',
            'Queso rallado, genérico',
            'Queso roquefort',
            'Queso San Simón',
            'Queso semicurado, genérico',
            'Queso serrat',
            'Queso servilleta',
            'Queso Tetilla',
            'Queso tierno, genérico',
            'Queso torta del casar',
            'Queso tupí',
            'Queso Zamorano',
            'Quinoa, cruda',
            'Rábano',
            'Rabo de toro',
            'Rape, a la parrilla',
            'Rape, crudo',
            'Rape, plancha',
            'Raya, cruda',
            'Refresco "tipo gaseosa"',
            'Refresco con gas, sabor cola, sin cafeina',
            'Refresco con gas, sabor cola, sin cafeina, bajo en calorías',
            'Refresco de té',
            'Refresco de té, bajo en calorías',
            'Refresco tipo "tónica"',
            'Refresco, con gas, sabor naranja',
            'Refresco, sabor cola',
            'Refresco, sabor cola, bajo en calorías',
            'Refresco, sabor limón, con gas',
            'Refresco, sabor naranja, sin gas',
            'Regaliz',
            'Remolacha, en conserva',
            'Repollo',
            'Requesón',
            'Riñón, de cordero, crudo',
            'Riñón, de ternera, crudo',
            'Rodaballo',
            'Rollito de primavera',
            'Romero',
            'Ron',
            'Rosquilla',
            'Rúcula',
            'Sal de mar',
            'Sal yodada',
            'Salami',
            'Salchicha de pollo fresca',
            'Salchicha, fresca',
            'Salchicha, tipo "frankfurt"',
            'Salchicha, tipo país, a la plancha',
            'Salchicha, tipo viena',
            'Salchichón',
            'Salema, cruda',
            'Salmón',
            'Salmón ahumado',
            'Salmón, plancha',
            'Salmonete',
            'Salsa agridulce',
            'Salsa al curry',
            'Salsa al roquefort',
            'Salsa barbacoa',
            'Salsa bechamel',
            'Salsa boloñesa',
            'Salsa carbonara',
            'Salsa de queso',
            'Salsa de soja',
            'Salsa napolitana',
            'Salsa siciliana, picante',
            'Salsa vinagreta, con aceite de oliva',
            'Salvado de trigo',
            'San jacobo, congelado',
            'Sandía',
            'Sangría',
            'Sardina',
            'Sardina, asada',
            'Sardina, enlatada, en aceite, escurrida',
            'Sardina, enlatada, en escabeche',
            'Sargo, crudo',
            'Seitán',
            'Semilla de lino',
            'Sémola de trigo, cruda',
            'Sémola de trigo, hervida',
            'Sepia',
            'Sésamo, semilla',
            'Sesos, de cerdo, crudos',
            'Sesos, de cordero, crudos',
            'Sesos, de ternera, crudos',
            'Seta',
            'Seta, plancha',
            'Sidra',
            'Sobao',
            'Sobrasada',
            'Soda',
            'Sofrito',
            'Soja, fresca',
            'Soja, frita',
            'Soja, germinada, en conserva',
            'Soja, seca, cruda',
            'Soja, seca, remojada, hervida',
            'Sopa de sobre maravilla',
            'Sopa de sobre, sin reconstituir',
            'Suplemento proteico',
            'Surimi',
            'Tabasco',
            'Tapioca, hervida',
            'Té, infusión, con leche',
            'Tenca, cruda',
            'Tequila',
            'Ternera, costilla, cruda',
            'Ternera, lomo, crudo, con grasa separable',
            'Ternera, parte s/e, asada',
            'Ternera, parte sin especificar, cruda, con grasa separable',
            'Ternera, solomillo, asado',
            'Ternera, solomillo, sin grasa, crudo',
            'Tocino',
            'Tofu',
            'Tomate',
            'Tomate frito',
            'Tomate, asado',
            'Tomate, maduro, pelado y triturado, enlatado',
            'Tomate, maduro, puré',
            'Tomillo, seco',
            'Torta de aceite',
            'Tortilla, a la francesa',
            'Trigo, entero, crudo',
            'Trucha',
            'Trucha, ahumada',
            'Trufa, cruda',
            'Turrón, tipo Alicante',
            'Uva blanca',
            'Uva blanca, congelada',
            'Uva negra, cruda',
            'Uva pasa',
            'Vaca/buey, parte s/e, asado',
            'Vaca/buey, parte s/e, estofado',
            'Vaca/buey, solomillo, a la plancha',
            'Vaca/buey, solomillo, crudo',
            'Vacuno, hígado, crudo',
            'Vacuno, mollejas, crudas',
            'Vainilla',
            'Vermut, s/e',
            'Vieira',
            'Vieja, cruda',
            'Vinagre balsámico',
            'Vinagre de manzana',
            'Vinagre de vino',
            'Vino blanco',
            'Vino dulce, tipo oporto',
            'Vino rosado',
            'Vino tinto',
            'Vodka',
            'Volador, crudo',
            'Whisky',
            'Yogur griego',
            'Yogur líquido, "tipo actimel"',
            'Yogur líquido, aromatizado sabor s/e',
            'Yogur líquido, con frutas s/e',
            'Yogur líquido, entero, con cereales',
            'Yogur líquido, natural, azucarado',
            'Yogur, búlgaro',
            'Yogur, desnatado, aromatizado sabor s/e',
            'Yogur, desnatado, con cereales',
            'Yogur, desnatado, con cereales, manzana y ciruela',
            'Yogur, desnatado, con cereza y frambuesa',
            'Yogur, desnatado, con ciruela, albaricoque y fibra',
            'Yogur, desnatado, con fresa, grosella y fibra',
            'Yogur, desnatado, con frutas',
            'Yogur, desnatado, con frutas del bosque',
            'Yogur, desnatado, con frutas tropicales',
            'Yogur, desnatado, con manzana',
            'Yogur, desnatado, con melocotón y maracuyá',
            'Yogur, desnatado, con melocotón, frambuesa y fibra',
            'Yogur, desnatado, con piña y pomelo',
            'Yogur, desnatado, sabor natural',
            'Yogur, desnatado, sabor natural, azucarado',
            'Yogur, desnatado, sabor vainilla',
            'Yogur, enriquecido, con frutas',
            'Yogur, enriquecido, natural',
            'Yogur, enriquecido, natural, azucarado',
            'Yogur, enriquecido, natural, con nata',
            'Yogur, enriquecido, sabor, s/e',
            'Yogur, entero, con cereales y fresas',
            'Yogur, entero, con fresas',
            'Yogur, entero, con frutas del bosque',
            'Yogur, líquido, desnatado, natural',
            'Yogur, líquido, entero, con fresas',
            'Yogur, líquido, entero, con frutas',
            'Yogur, líquido, entero, sabor fresa',
            'Yogur, líquido, entero, sabor fresa y plátano',
            'Yogur, líquido, entero, sabor frutas del bosque',
            'Yogur, líquido, entero, sabor piña y coco',
            'Zamburiñas',
            'Zanahoria, cruda',
            'Zumo de grosella negra',
            'Zumo de grosella roja',
            'Zumo de lima, envasado',
            'Zumo de limón, fresco',
            'Zumo de mango',
            'Zumo de manzana',
            'Zumo de naranja',
            'Zumo de piña',
            'Zumo de piña y uva',
            'Zumo de pomelo, envasado',
            'Zumo de tomate, fresco',
            'Zumo de uva',
            'Zumo de uva y melocotón',
            'Zumo de zanahoria, fresco',
    );

    $arrLength = count($ingredients);
    for ($i = 0; $i < $arrLength; $i++){
            
            $ingrediente = new Ingredient();
            $ingrediente->setName($ingredients[$i]);
            $manager->persist($ingrediente);
            $this->addReference(self::INGREDIENT_REFERENCE . $i, $ingrediente);
    }

        $manager->flush();
    }
}
